...
const SCholesterolForm = ({ data, clearScreen }: { data: DataEmptyTests, clearScreen: (testRegisterId: number, testId: number) => void }) => {
    ...
    const onFinish = async (values: any) => {
        try {
            const savingData = {
                sCholesterolValue: Number(values.sCholesterolValue),
                sCholesterolValueFlag: values.sCholesterolValueFlag,
    ...
    return (
        <div className="w-full">
                ...
                initialValues={
                    {
                        ...
                        "sCholesterolValue": data.data?.sCholesterolValue,
                        "sCholesterolValueFlag": data.data?.sCholesterolValueFlag,
            ...
            >
                ...
                <Form.Item label="S. Cholesterol" style={{ marginBottom: 0 }}>
                    <Form.Item
                        name="sCholesterolValue"
                        rules={[{ required: true }]}
                        style={{ display: 'inline-block', width: '200px' }}
                    >
                        <Input addonAfter="mg/dl" placeholder="value" onChange={(e) => setFlag('sCholesterolValue', e.target.value, investigationFieldsResults?.content, normalRangeRulesResults?.content, data.patientDOB, data.patientGender, form)} />
                    </Form.Item>
                    <div className="flex-row items-center inline-flex">
                        <Form.Item
                            name="sCholesterolValueFlag"
                        ...
                        <span>
                            {displayNormalRange('sCholesterolValue', investigationFieldsResults?.content, normalRangeRulesResults?.content, data.patientDOB, data.patientGender)}
                        </span>
                    </div>
                </Form.Item>
...
export default SCholesterolForm;