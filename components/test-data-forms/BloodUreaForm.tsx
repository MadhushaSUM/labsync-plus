...
const BloodUreaForm = ({ data, clearScreen }: { data: DataEmptyTests, clearScreen: (testRegisterId: number, testId: number) => void }) => {
    ...
    const onFinish = async (values: any) => {
        try {
            const savingData = {
                bloodUreaValue: Number(values.bloodUreaValue),
                bloodUreaValueFlag: values.bloodUreaValueFlag,
    ...
    return (
        <div className="w-full">
                ...
                initialValues={
                    {
                        ...
                        "bloodUreaValue": data.data?.bloodUreaValue,
                        "bloodUreaValueFlag": data.data?.bloodUreaValueFlag,
            ...
            >
                ...
                <Form.Item label="Blood Urea" style={{ marginBottom: 0 }}>
                    <Form.Item
                        name="bloodUreaValue"
                        rules={[{ required: true }]}
                        style={{ display: 'inline-block', width: '200px' }}
                    >
                        <Input addonAfter="mg/dl" placeholder="value" onChange={(e) => setFlag('bloodUreaValue', e.target.value, investigationFieldsResults?.content, normalRangeRulesResults?.content, data.patientDOB, data.patientGender, form)} />
                    </Form.Item>
                    <div className="flex-row items-center inline-flex">
                        <Form.Item
                            name="bloodUreaValueFlag"
                        ...
                        <span>
                            {displayNormalRange('bloodUreaValue', investigationFieldsResults?.content, normalRangeRulesResults?.content, data.patientDOB, data.patientGender)}
                        </span>
                    </div>
                </Form.Item>
                <Form.Item
