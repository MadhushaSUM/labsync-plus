...
const RCholesterolForm = ({ data, clearScreen }: { data: DataEmptyTests, clearScreen: (testRegisterId: number, testId: number) => void }) => {
    ...
    const onFinish = async (values: any) => {
        try {
            const savingData = {
                rCholesterolValue: Number(values.rCholesterolValue),
                rCholesterolValueFlag: values.rCholesterolValueFlag,
    ...
    return (
        <div className="w-full">
                ...
                initialValues={
                    {
                        ...
                        "rCholesterolValue": data.data?.rCholesterolValue,
                        "rCholesterolValueFlag": data.data?.rCholesterolValueFlag,
            ...
            >
                ...
                <Form.Item label="Random Cholesterol" style={{ marginBottom: 0 }}>
                    <Form.Item
                        name="rCholesterolValue"
                        rules={[{ required: true }]}
                        style={{ display: 'inline-block', width: '200px' }}
                    >
                        <Input addonAfter="mg/dl" placeholder="value" onChange={(e) => setFlag('rCholesterolValue', e.target.value, investigationFieldsResults?.content, normalRangeRulesResults?.content, data.patientDOB, data.patientGender, form)} />
                    </Form.Item>
                    <div className="flex-row items-center inline-flex">
                        <Form.Item
                            name="rCholesterolValueFlag"
                        ...
                        <span>
                            {displayNormalRange('rCholesterolValue', investigationFieldsResults?.content, normalRangeRulesResults?.content, data.patientDOB, data.patientGender)}
                        </span>
                    </div>
                </Form.Item>
...
export default RCholesterolForm;